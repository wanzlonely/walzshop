...
export async function POST(request) {
  ...
  if (endpoint === 'ppob_inquiry') {
    if (!user) return NextResponse.json({ success: false, msg: 'Login dulu' }, { status: 401 });
    const { product_code, customer_id, server_id = '' } = payload;
    const cat = (payload.category || '').toLowerCase();
    if (cat.includes('pascabayar')) {
      try {
        const { tvPascabayarInquiry } = await import('@/lib/tokovoucher');
        const refId = `INQ-${Date.now()}-${user.id.slice(-4)}`;
        const result = await tvPascabayarInquiry(refId, product_code, customer_id, server_id);
        return NextResponse.json({ success: true, data: result?.data || result });
      } catch(e) {
        return NextResponse.json({ success: false, msg: 'Gagal inquiry tagihan' });
      }
    }
    return NextResponse.json({ success: true, data: { customer_id, product_code } });
  }

  if (endpoint === 'ppob_pay') {
    if (!user) return NextResponse.json({ success: false, msg: 'Login dulu' }, { status: 401 });
    const { product_code, customer_id, server_id = '', harga, namaProduk = '' } = payload;
    const hargaNum = Number(harga);
    if (!hargaNum) return NextResponse.json({ success: false, msg: 'Harga tidak valid' });
    const saldo = Number(user.balance || 0);
    if (saldo < hargaNum) return NextResponse.json({ success: false, msg: `Saldo tidak cukup`, error_code: 'INSUFFICIENT_BALANCE', required: hargaNum });
    const refId = `TV-${Date.now()}-${user.id.slice(-6).toUpperCase()}`;
    await deductBalance(user.id, hargaNum);
    try {
      const { tvTransaksi } = await import('@/lib/tokovoucher');
      const tvResult = await tvTransaksi(refId, product_code, customer_id, server_id);
      const tvStatus = tvResult?.data?.status || 'pending';
      const sn = tvResult?.data?.sn || '';
      await redis.hset(`ppob:${refId}`, { userId: user.id, produk: product_code, namaProduk, tujuan: customer_id, harga: hargaNum, status: tvStatus === 'sukses' ? 'sukses' : 'pending', sn, createdAt: Date.now() });
      if (tvStatus === 'gagal') await addBalance(user.id, hargaNum);
      return NextResponse.json({ success: true, refId, status: tvStatus, sn, message: tvResult?.message || 'Transaksi diproses' });
    } catch(e) {
      await addBalance(user.id, hargaNum);
      return NextResponse.json({ success: false, msg: 'Gagal terhubung ke server' });
    }
  }
